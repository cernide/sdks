...
import { V1LogFromJSON, V1LogFromJSONTyped, V1LogToJSON } from "./V1Log";
...
export function V1LogsFromJSONTyped(
    json: any,
    ...
    return {
        logs: !exists(json, "logs")
            ? undefined
            : (json["logs"] as Array<any>).map(V1LogFromJSON),
        last_time: !exists(json, "last_time")
            ? undefined
            : new Date(json["last_time"]),
    ...
    return {
        logs:
            value.logs === undefined
                ? undefined
                : (value.logs as Array<any>).map(V1LogToJSON),
        last_time:
            value.last_time === undefined
                ? undefined
                : value.last_time.toISOString(),

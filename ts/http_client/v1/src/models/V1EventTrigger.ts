...
export function V1EventTriggerFromJSONTyped(
    json: any,
    ...
    return {
        kinds: !exists(json, "kinds")
            ? undefined
            : (json["kinds"] as Array<any>).map(V1EventKindFromJSON),
    ...
    return {
        kinds:
            value.kinds === undefined
                ? undefined
                : (value.kinds as Array<any>).map(V1EventKindToJSON),
        ref: value.ref,
    };
}
